...
```c
typedef struct AMQPTRANSPORT_DEVICE_CONFIG_TAG
{


} AMQPTRANSPORT_DEVICE_CONFIG;

typedef void* AMQPTRANSPORT_DEVICE_HANDLE;

typedef enum AMQPTRANSPORT_ERROR
{
    AMQPTRANSPORT_ERROR_GENERAL_FAILURE;
}
...
typedef int (*FP_AMQPTRANSPORT_GET_SEND_STATUS)(AMQPTRANSPORT_DEVICE_HANDLE device_handle);
typedef SESSION_HANDLE (*FP_AMQPTRANSPORT_GET_SESSION)(AMQPTRANSPORT_CORE_HANDLE handle);
...
struct AMQPTRANSPORT_CORE_INTERFACE_TAG
{
    ...
    FP_AMQPTRANSPORT_GET_SESSION get_session;
...
AMQPTRANSPORT_CORE_HANDLE amqptransport_core_cbs_device_key_create(const AMQPTRANSPORT_CORE_CBS_KEY_CONFIG* config);
